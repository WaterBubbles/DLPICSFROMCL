...
#qianzhui="htm_data/16/1702"
for yeshu in range(1,2):
    address='http://www.t66y.com/thread0806.php?fid=16&search=&page='+str(yeshu)
    res=requests.get(address,headers=my_headers)
    soup = BeautifulSoup(res.text,'lxml')

    for res in soup.find_all('a'):
        res1=str(res.get('href'))
        if(res1[0:16]=='htm_data/1911/16'):
